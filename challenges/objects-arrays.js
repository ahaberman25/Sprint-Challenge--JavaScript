...
for (i = 0; i < graduates.length; i++) {
  const universities = graduates[i].university;
...
zooAnimals.filter((animal) => {
  if (lowPop = animal.population < 5) {
    return lowPopulationAnimals.push(animal.animal_name);
  }  
});
...
zooAnimals.reduce((accum, population) => {
  return populationTotal = accum + population.population;
}, 0);
